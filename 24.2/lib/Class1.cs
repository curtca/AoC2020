...
public class Floor
{
    ...
    public int Black()
    {
        ...
        for (int day = 1; day <= 100; day++)
        {
            ...
            */
            ...
            floor = Expand();
            Dictionary<(int, int), bool> newfloor = new Dictionary<(int, int), bool>(floor);
            foreach (var tile in floor)
            {
                int blackneighbors = BlackNeighbors(tile.Key);
                if (tile.Value && (blackneighbors == 0 || blackneighbors > 2))
                    newfloor[tile.Key] = false;
                else if (!tile.Value && (blackneighbors == 2))
                    newfloor[tile.Key] = true;
            }
            black = newfloor.Sum(t => t.Value ? 1 : 0);
            Debug.Print($"Day {day}: {black}");
            floor = newfloor;
        }
    ...
    private int BlackNeighbors((int, int) loc)
    {
        int bn = 0;
        foreach (var dir in hexdirs.Values)
        {
            (var q, var r) = loc;
            q += dir.Item1;
            r += dir.Item2;
            bool blackneighbor = false;
            if (floor.TryGetValue((q, r), out blackneighbor))
                if (blackneighbor) bn++;
        }
        return bn;
    }

    private Dictionary<(int, int), bool> Expand()
    {
        Dictionary<(int, int), bool> newfloor = new Dictionary<(int, int), bool>(floor);
        foreach (var tile in floor.Keys)
        {
            foreach (var dir in hexdirs.Values)
            {
                ...
                q += dir.Item1;
                r += dir.Item2;
                if (!newfloor.ContainsKey((q, r)))
                    newfloor[(q, r)] = false; // white
            }
        }
        return newfloor;
    }

    private void StartingFloor()
    {
